...
@Component
@AllArgsConstructor
public class GetTickerForSubscribe implements Command {
    @Override
    public List<BaseRequest<?, ? extends BaseResponse>> apply(Update update, UserRepository repository) {
        ...
        try {
            ...
            if(!message.equals("Отмена")){
                repository.subscribe(id,message);
                repository.setState(id, State.NONE);
                return List.of(new SendMessage(id, "Вы подписались на тикер" + message).replyMarkup(remove));
            }
            else{
                repository.setState(id, State.NONE);
